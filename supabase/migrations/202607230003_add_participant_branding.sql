-- Add client branding (accent colour + logo) to the public participant view.
-- Both are intentionally participant-facing; every other organization field stays
-- gated. Idempotent: replaces the function in place.

begin;

create or replace function public.get_participant_simulation(public_token text)
returns jsonb
language sql
stable
strict
...
as $$
  select jsonb_strip_nulls(
    jsonb_build_object(
      ...
      'accentColor', nullif(simulation.organization ->> 'accentColor', ''),
      'logo', nullif(simulation.organization ->> 'logo', ''),
      'participantBrief',
        jsonb_build_object(
